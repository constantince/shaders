...
#define START 0.0
#define END 50.0
#define PRECISON 0.01
#define PI 3.1415926535

uniform vec2 u_resolution;
uniform float u_time;
unifomr vec2 u_mouse;
...
Items sdSphere(vec3 p, vec3 offset, float radius, Material m, int id) {
    return Items(length(p - offset) - radius, id, m);
}

Items sdFloor(vec3 p, Materail m, int id) {
...
Material gold() {
    vec3 aCol = 0.5 * vec3(0.7, 0.5, 0.0);
    vec3 dCol = 0.6 * vec3(0.7, 0.7, 0.0);
    vec3 sCol = 0.6 * vec3(1.0, 1.0, 1.0);
    float a = 5;
...
Items assembleItems(vec3 p) {
    ...
    Items floors = sdFloor(p, floorMaterial(p), 2);
